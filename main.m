...
syms  Kh dh           % Trunk Stiffness , Trunck Damping 
...
%          (Js_int*tetadd_ext - Kh*teta_h + Kh*teta_ext - Ks*teta_int + Ks*teta_ext - dh*tetad_h + dh*tetad_ext - ds*tetad_int + ds*tetad_ext)/rg == 0 ,
%          -(tau_h - Kh*teta_h + Kh*teta_ext - dh*tetad_h + dh*tetad_ext - tetadd_h*(Mh*rh^2 + Jh) + Mh*g*rh*sin(teta_h))/rg == 0 ];

eqns = [ J_act*TETADD_ACT + (Kg*teta_act + dg*tetad_act - rg^2*tau_m + bm*rg^2*tetad_act - Kg*rg*teta_int - dg*rg*tetad_int)/rg^2 == 0 ,
         (Js_int*TETADD_INT - (Kg*teta_act + dg*tetad_act - Kg*rg*teta_int - Ks*rg*teta_int + Ks*rg*teta_ext - dg*rg*tetad_int - ds*rg*tetad_int + ds*rg*tetad_ext)/rg)/rg == 0 ,
         (Js_int*TETADD_EXT - Kh*teta_h + Kh*teta_ext - Ks*teta_int + Ks*teta_ext - dh*tetad_h + dh*tetad_ext - ds*tetad_int + ds*tetad_ext)/rg == 0 ,
         -(0 - Kh*teta_h + Kh*teta_ext - dh*tetad_h + dh*tetad_ext - TETADD_H*(Mh*rh^2 + Jh) + Mh*g*rh*teta_h)/rg == 0 ];
...
Trq_s = solve( Trq_s == [ (Trq_d - (Trq_s/rg))*(ctrl) + Trq_d ]*TF(1)  +  [ (Trq_d - (Trq_s/rg))*(ctrl) + (vel_h*dh + vel_h*s*Mh*(rh^2) + Jh*vel_h*s ) ]*TF(2) , Trq_s);
